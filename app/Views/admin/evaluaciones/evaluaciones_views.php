...
<script>
    ...
    function eliminarEvaluacion(id) {
        if (confirm('¿Estás seguro de que quieres eliminar esta evaluación?')) {
            fetch(`<?= base_url('admin/evaluaciones/eliminar') ?>/${id}`, {
                method: 'POST'
            })
            .then(response => response.json())
            .then(data => {
    ...
    function aplicarFiltrosEvaluaciones() {
        ...
        fetch('<?= base_url('admin/evaluaciones/filtros') ?>', {
            method: 'POST',
            body: formData
        })
        .then(response => response.json())
    ...
    function exportarPDF() {
        const url = '<?= base_url('admin/reportes-evaluaciones/pdf') ?>';
        window.open(url, '_blank');
        showNotification('Generando reporte PDF...', 'info');
    }

    function exportarExcel() {
        const url = '<?= base_url('admin/reportes-evaluaciones/excel') ?>';
        window.open(url, '_blank');
        showNotification('Exportando a Excel...', 'info');
    }

    function exportarCSV() {
        const url = '<?= base_url('admin/reportes-evaluaciones/csv') ?>';
        window.open(url, '_blank');
    ...
    function cargarEstadisticas() {
        fetch('<?= base_url('admin/evaluaciones/estadisticas') ?>')
            .then(response => response.json())
            .then(data => {
                if (data.success) {
