        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="<?= base_url('coord/perfil/update') ?>" method="post" id="formPerfil">
                            <?= csrf_field() ?>
                            <div class="row">
...
<script>
    <?php $csrfName = config('Security')->tokenName ?? 'csrf_test_name';
    $csrfValue = csrf_token(); ?>
    var csrfTokenName = <?= json_encode($csrfName) ?>;
    var csrfTokenValue = <?= json_encode($csrfValue) ?>;
    var uploadUrl = <?= json_encode(base_url('coord/perfil/upload-image')) ?>;
    var deleteImageUrl = <?= json_encode(base_url('coord/perfil/delete-image')) ?>;
    ...
    function eliminarFoto() {
        ...
        fetch(deleteImageUrl, {
            ...
            .finally(function() {
                if (btn) {
                    btn.disabled = false;
                    btn.innerHTML = '<i class="fas fa-trash-alt me-1"></i>Eliminar foto';
                }
            });
    }
